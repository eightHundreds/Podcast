...
Single show:
  python scripts/generate_feed.py -i shows/…/podcast.yaml -o docs/ddia/feed.xml
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "--output",
        type=Path,
        help="输出 XML（单节目模式；默认 docs/ddia/feed.xml）",
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="按 shows.yaml 为每个节目生成 feed（docs/<slug>/feed.xml）",
    )
    parser.add_argument(
        "--index",
        type=Path,
    ...
    output = args.output or (REPO_ROOT / "docs" / "ddia" / "feed.xml")
    generate_one(
        Path(input_path),
        Path(output),
